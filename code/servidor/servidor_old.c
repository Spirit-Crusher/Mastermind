...
{   
    ...
    while (state == ONGOING)
    {
        ...
        {
            case 0:
                sem_wait(&sem_games[0]);
                break;
            
            case 1:
                sem_wait(&sem_games[1]);
                break;

            case 2:
                sem_wait(&sem_games[2]);
                break;

            case 3:
                sem_wait(&sem_games[3]);
                break;
...
void stream_handler(int socket_descriptor)
{
    ...
    switch (buffer_stream.command)
    {
        ...
        case JG:
            ...
            if (game_number != -1)
            {
                sem_post(&sem_games[game_number]);
            }
            else
            {
                printf("[ERRO] Game number inválido: %d\n", game_number);
...
int main() {
    ...
    for(int i = 0; i < NJMAX; ++i)
    {
        if(sem_init(&sem_games[0], 0, 0) != 0)
        {
            printf("[ERRO] Criação do semáforo %d falhou", i);
        }
    }
